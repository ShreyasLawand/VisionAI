...
weights_path = r"C:\Users\Shreyas Lawand\OneDrive\Desktop\VisionAI\VisionAI\yolov5s.pt"  # Update with the path to the downloaded YOLOv5 weights
...
def recognize_faces():
    ...
    datasets = r"C:\Users\Shreyas Lawand\OneDrive\Desktop\VisionAI\VisionAI\datasets"
...
def execute_command(command):
    if command == "recognition":
        ...
        cap = cv2.VideoCapture(url)  # Use webcam source 0
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
                break
